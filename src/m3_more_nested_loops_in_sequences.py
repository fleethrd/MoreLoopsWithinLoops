...
def run_test_largest_number():
    ...
    answer = largest_number(([1, 2, 3], [], [1000000000, 10000000000]))
    print('Expected and actual are:', expected, answer)
...
def run_test_largest_negative_number():
    ...
    # Test 4:
    expected = -1
    answer = largest_negative_number(([-1, 2, -3], [], [-1000000000, 10000000000]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
            if track == 0 and seq_seq[k][j] < 0:
                large = seq_seq[k][j]
                track = 1
            else:
                if seq_seq[k][j] > large and seq_seq[k][j] < 0:
                    large = seq_seq[k][j]
    if large:
        return large
    else:
